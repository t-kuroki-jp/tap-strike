...
### 📌 ルール3: カラーコード ✕ パラメータ相関ルール (Color-Coding System)
ドラゴンクエストの「スライム（青・標準） ➔ スライムベス（赤・強化/速度）」のように、**ノーツの「ネオンカラー」によってプレイヤーが直感的にパラメータや危険度を察知できるカラーリング法則** を適用します。

| ネオンカラー | カラー系統 | 暗示するパラメータ / 属性 | 具体例・役割 |
| :--- | :--- | :--- | :--- |
| 🔴 **レッド** | `#ff3366` | **標準 (Standard)** | チェイサー (`speedRatio: 1.0`, `hp: 1`) |
| 💛 **イエロー** | `#ffff33` | **ハイスピード (High-Speed)** | スピーダー (`speedRatio: 1.5`), リング (`orbit`) |
| 🔵 **ブルー** | `#0088ff` | **高耐久・シールド (Defense)** | シールドノーツ (`hp: 2`) |
| 🟣 **パープル** | `#cc00ff` | **トリッキー・ボス (Boss / Glitch)** | グリッチ (直前減速), ビッグボス (`hp: 5`) |
| 🟢 **グリーン/シアン** | `#00ffcc` | **特殊アプローチ (Wave / Special)** | サイン・ウェイバー (波状), バウンド・ペンタ |
| 💖 **ピンク** | `#ff0077` | **フェイント・回転 (Feint / Acrobat)**| ブーメラン・クロス (引き返し回転) |
| 🤍 **ライム/ホワイト**| `#aaff66` | **ステルス・隠密 (Stealth / Ghost)**| ステルス・ゴースト (透明化) |

> 💡 **色違いバリエーション展開の指針**:
> 同じ幾何学形状（例: 丸ノーツ `Circle`）であっても、色を変えることで「赤丸＝通常速度」「黄丸＝1.5倍速」「青丸＝HP2耐久」というように、視覚的バリエーション（スライムベス方式）を無制限に拡張可能です。
