...
新しいステージを作成・編集する際、個別のステージ JSON が持つべき基本的な標準プロパティは **以下の 5 つのみ** です。共通の物理パラメータ (`params` / `spawnRate`) やカラーテーマ (`theme`) は `stages/index.json` のマスタから自動補填されるため省略可能です（特殊な演出・ペースの時のみ指定してオーバーライドします）。
