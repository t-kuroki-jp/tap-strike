...
> マスター登録簿 `stages.json` の `"funny"` 配列に書かれた並び順そのままで画面に表示され、**一番上（先頭 3 ステージ）に自動で金色の `NEW!` バッジ** が付与されます。新しいステージを `stages.json` の `"funny"` 配列の先頭（上）に追加していく運用です。日付等の手書き属性は一切不要です。
...
2. **マスターリスト (`stages.json`) への登録**:
   - プロジェクト直下の `stages.json` の配列に、作成したファイルの相対パス（例: `"stages/funny/my_new_stage.json"`）を 1 行追加します。
