...
| ↳ **`sakuraEffect`**| `boolean`| *任意* | `true` の場合、背景および画面下部にリアルな夜桜吹雪が咲き乱れる。 | `true` (夜桜の舞) |
| ↳ **`bubbleEffect`**| `boolean`| *任意* | `true` の場合、画面背景を幻想的な水泡アクアリウム演出に変化。 | `true` (ウミウシ) |
| ↳ **`rainbow`** | `boolean`| *任意* | `true` の場合、画面背景全域が七色レインボーグラデーションに変色。 | `true` (レインボー) |
...
| ↳ **`baseScore`**| `number` | *任意* | ノーツ 1 体あたりの基礎獲得スコア。 | `10` |
